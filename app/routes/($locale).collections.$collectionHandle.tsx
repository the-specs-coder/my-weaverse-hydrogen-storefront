...
export default function Collection() {
  const { collection } = useLoaderData<typeof loader>();
    ...
    useEffect(() => {
    const searchParams = new URLSearchParams(location.search);
    const existingParams = JSON.parse(localStorage.getItem("storedParams") || "{}");

    // Loop through the new params
    searchParams.forEach((value, key) => {
      // Initialize array for each key if not present
    ...
    // Save updated params back to localStorage
    localStorage.setItem("storedParams", JSON.stringify(existingParams));
  }, [location.search]);

  // const params = JSON.parse(window.localStorage.getItem('myQueryParams'));
  // console.log(params);
